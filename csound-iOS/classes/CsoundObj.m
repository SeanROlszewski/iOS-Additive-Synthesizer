...
-(id<CsoundValueCacheable>)addKnob: (UIKnob *)uiKnob forChannelName:(NSString *)channelName
{
    CachedKnob *cachedKnob = [[[CachedKnob alloc] init:uiKnob channelName:channelName] autorelease];
    
    [valuesCache addObject: cachedKnob];
    return cachedKnob;
}
...
-(void)sendScoreEvent: (char)eventType withPFieldValues: (MYFLT *)pFieldValues forNumberPFields: (int)pFieldCount {
    if (mCsData.cs != NULL) {
        csoundScoreEvent(mCsData.cs, eventType, pFieldValues, pFieldCount);
    }
}
